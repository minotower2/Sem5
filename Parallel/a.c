...
int main(int argc, char ** argv) {
  ...
  int n, p, k, res = 0, result, task = 1, total_threads = 1;
  char *name = 0;
  pthread_t* threads;
  ARGS * args;
  ...
  if (!(
    ...
    sscanf(argv[2], "%d", &total_threads) == 1 && total_threads > 0 && 
    sscanf(argv[3], "%d", &p) == 1 && p >= 0 && 
    sscanf(argv[4], "%d", &k) == 1 && k >= 0 && k < 5)) 
  {
    printf("Usage %s size_of_matrix number_of_threads ammount_of_shown_elements formula (filename)\n", argv[0]);
    return 1;
  } 
  ...
  if (!threads) {printf("Not enough memory!\n"); free(a); free(x); free(a_rev); return 2;}
  args = (ARGS*) malloc (total_threads * sizeof (ARGS));
  if (!args) {printf("Not enough memory!\n"); free(a); free(x); free(a_rev); free(threads); return 2;}
